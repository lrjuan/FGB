	...
	public void add_Externals(String[] tracks,String[] links,String[] types,String[] modes){
		...
		for(int i=0;i<tracks.length;i++)
			if(!Externals.containsKey(tracks[i])){
				...
				if(!validurl)
					((Annotations) Externals.get(tracks[i])).set_Check("The FGB cannot access the data file! The following file(s) may not be publicly accessible: "+invalidlist);
			}
	}
...
//				Pvar.set_Mode(mode);
	...
	public String get_TrioAnalysis(String track, String id, String chr, long start, long end){
		...
		if(chrid>=0){
			...
			if(vt!=null&&vt.get_Type().equals(Consts.FORMAT_VCF)&&vt.has_Parameter(VCF_HEADER_SAMPLE)){
				VcfReader vr = new VcfReader(vt, chr);
				Element[] ele_temp = vr.write_trio(doc, id, chr, start, end, false, false);
			}
		}
		return XmlWriter.xml2string(doc);
	}
