...
*/				
	...
	public Element write_intersection(Document doc, String track, String set_a, String chr, long start, long end){
		...
		if (this.track.has_Parameter(VCF_HEADER_SAMPLE)) {
			...
			splitcolon.split(set_a);
			ref_index = vcfSample.getSelectedIndex(splitcolon.getResultByIndex(splitcolon.getResultNum()-1));
			//get selected index of reference individual
		}
			...
			if (Query != null) {
				...
				while (Query.next() != null) {
					...
					if (!vcf.isDBSnp() && siNotNull) {
						// Personal Genemic VCF
						vs = vcf.getVariants_intersection(selectedIndexes.length,ref_index);
						if (vs != null){
							if(end-start<BinMethodThreshold){
								variants[0].addVariant(vcf,-1, vs);
							}
							else{
								String homo = vs[0].getHomo();
								if(Vcf.containChar(homo, '|')){
									splitpipe.split(homo);
									if(!splitpipe.getResultByIndex(0).equals("0"))
										var_value_list[0][(int)((vs[0].getFrom()-start)/step)]++;
									if(!splitpipe.getResultByIndex(1).equals("0"))
										var_value_list[1][(int)((vs[0].getFrom()-start)/step)]++;
								}
		...
		Element e1;
		if(end-start<BinMethodThreshold){
			e1 = variants[0].getVariantsElement();
		}
		else{
			doc.getElementsByTagName(DATA_ROOT).item(0).removeChild(variants[0].getVariantsElement());
			e1 = var_value2xml(doc, track, "IntersectionL",var_value_list[0], start, end, (int)step); // Left
			e1 = var_value2xml(doc, track, "IntersectionR",var_value_list[1], start, end, (int)step); // Right
		}
		variants = null;
		return e1;
	}
	public Element write_difference(Document doc, String track, String set_a, String set_b, String chr, long start, long end){
		...
		try {
			...
			if (Query != null) {
				...
				while (Query.next() != null) {
					...
					if (!vcf.isDBSnp() && siNotNull) {
						// Personal Genome VCF
						vs = vcf.getVariants_difference(selectedIndexes_a,selectedIndexes_b,ref_index);
						if (vs != null){
							if(end-start<BinMethodThreshold){
								variants[0].addVariant(vcf,-1, vs);
							}
		...
		if(end-start<BinMethodThreshold){
			...
			doc.getElementsByTagName(DATA_ROOT).item(0).removeChild(variants[0].getVariantsElement());
			e1 = var_value2xml(doc, track, "DifferenceL",var_value_list[0], start, end, (int)step); // Left
			e1 = var_value2xml(doc, track, "DifferenceR",var_value_list[1], start, end, (int)step); // Right
		}
		variants = null;
		return e1;
	}
	public Element[] write_trio(Document doc, String oid, String chr, long start, long end, boolean scan){
		...
		if(scan || end - start < BinMethodThreshold){
			...
			doc.getElementsByTagName(DATA_ROOT).item(0).removeChild(variants[f].getVariantsElement());
			doc.getElementsByTagName(DATA_ROOT).item(0).removeChild(variants[m].getVariantsElement());
			e[1] = var_value2xml(doc, track.get_ID(), pids[0]+"L",var_value_list[0], start, end, (int)step); // Left
			e[1] = var_value2xml(doc, track.get_ID(), pids[0]+"R",var_value_list[1], start, end, (int)step); // Right
			e[2] = var_value2xml(doc, track.get_ID(), pids[1]+"L",var_value_list[2], start, end, (int)step); // Left
			e[2] = var_value2xml(doc, track.get_ID(), pids[1]+"R",var_value_list[3], start, end, (int)step); // Right
		}
		return e;
	}
...
//			e.printStackTrace();
		...
		Element e1;
		if(end-start<BinMethodThreshold || vcfSample == null || vcfSample.getSelectedNames() == null || vcfSample.getSelectedNames().length == 0){
			e1 = variants[0].getVariantsElement();
		}
		else{
			doc.getElementsByTagName(DATA_ROOT).item(0).removeChild(variants[0].getVariantsElement());
	...
	static Element var_value2xml(Document doc, String track, String id, int[] var_value_list, long start, long end, int step){
		...
		XmlWriter.append_text_element(doc, ele, Consts.XML_TAG_FROM, start + "");
		...
		XmlWriter.append_text_element(doc, ele, Consts.XML_TAG_VALUE_LIST, valueList.toString());
		return ele;
	}
