...
function plot_families(){
	...
	if(trackname == "1000genomes_phase3_related_individuals_autosomes" && default_flag == 0){
		var urlParameter = QueryString();
		if(urlParameter["Chr"] != null && parseInt(urlParameter["Start"]) != null && parseInt(urlParameter["End"]) != null){
			url_chrnum = urlParameter["Chr"];
			url_start = parseInt(urlParameter["Start"]);
			url_end = parseInt(urlParameter["End"]);
		}
	//	window.location.href = "browser.html";
		families[default_id].obj[default_root][0].onclick();
		default_flag = 1;
	}
...
function select_a_individual(id){
	if(individuals[id] != undefined){
		...
		if(individuals[id].selected){
			...
			setTabb("brwview");
			if(url_chrnum != "" && url_start != 0 && url_end != 0){
				load_family_genome(url_chrnum,url_start,url_end);
			}else{
