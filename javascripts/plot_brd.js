...
//		B.rect(80,h*(chrs[idx].from+chrs[idx].centromere)/total,60,2,0).attr(attr);
	...
	var scan_text2 = B.text(166,15,"de novo").attr({font: "italic 16px Trebuchet MS, Arial, sans-serif",opacity:1}).attr({fill: "#000"});
	...
	if(individuals[csi] != undefined && individuals[individuals[csi].fid] != undefined && individuals[individuals[csi].mid] != undefined && individuals[individuals[csi].fid].ifs == "true" && individuals[individuals[csi].mid].ifs == "true"){
		btn_set.show();
	}else{
		btn_set.hide();
	}
